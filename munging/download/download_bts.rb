...
Settings.define :tmp_dir, description: "Place to temporarily put the zip archives before extraction", default: "#{Dir.pwd}/.tmp"
Settings.resolve!
...
def mkdir_unless_exists(dir,local=true)
  mkdir(dir,local) unless dir_exists?(dir,local)
end
...
def download(year,month,out_dir)
  ...
  out_file = "#{out_dir}/#{year}-#{month}.zip"
  puts "Downloading #{month} #{year} to #{out_file}"
  `#{curl} > #{out_file}`
end
...
if Settings.local and Settings.out_dir = '/data/origin/www.transtats.bts.gov'
...
Settings.years.each do |year|
  ...
  mkdir_unless_exists(out_dir, Settings.local)
  Settings.months.each_with_index do |month,index|
    download(year,month,Settings.tmp_dir)
    unzip(year,month,Settings.tmp_dir,out_dir) 
  end
end
`rm -r #{Settings.tmp_dir}`
