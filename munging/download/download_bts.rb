...
Settings.define :years, flag: 'y', description: 'Retrieve these years', type: Array, default: YEARS
Settings.define :months, flag: 'm', description: "Retrieve these months", type:Array, default: MONTHS
...
Settings.define :hdfs_out, description: "Directory in which to put the files on the hdfs", default: '/data/raw'
...
def transform(filename)
   `#{$sed} 's/"//g' #{filename}`  #remove quotes
   `#{$sed} 's/,$//g' #{filename}`  #remove trailing commas
   `#{$sed} '1d' #{filename}` if Settings.remove_header  #remove header
end

def to_tsv(filename)
  out_filename = "#{filename[0..-4]}tsv"
  `#{$sed} 's/,/	/g' #{filename}` #remove commas
